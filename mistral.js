class MistralPredictor {
    ...
    async getPrediction(text) {
        try {
            ...
            const prompt = `Ты - система предиктивного ввода (Т9). Как SwiftKey твоя задача - предлагать релевантные подсказки в виде слов с точным соблюдением грамматики русского языка.
...
2. Анализ управления:
   а) Если есть глагол:
      ...
      - "сказать (что?) привет" -> именительный падеж
      - "нет (чего?) смысла" -> родительный падеж
      - "верить (чему?) идее" -> дательный падеж
